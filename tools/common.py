...
# ISO 3166-1 alpha-2 is the join key for every country-shaped thing here: a record's
# facets.iso2, a World Bank row, a Natural Earth polygon, a price, a rule. Two letters,
# uppercase, and nothing else is accepted.
...
def country_by_geo(lat: float, lon: float, detail: bool = False):
    """An ISO 3166-1 alpha-2 code, or None, by point-in-polygon against
    data/geo/countries.json (ray casting). A harvested row's own addr:country tag is a
    better tell; this is the fallback for the rows that carry none. Bounding boxes are
    checked first so a point in Malaysia does not walk the Canadian arctic.

    detail=True reads the 1:50m outlines instead of the 1:110m ones the maps are drawn
    from: at 1:110m Singapore has no polygon at all and Geneva falls in France."""
    key = "detail" if detail else "coarse"
    if key not in _COUNTRY_RINGS:
        path = GEO / ("countries-detail.json" if detail else "countries.json")
        if not path.exists():
            path = GEO / "countries.json"
        g = jload(path) if path.exists() else {"countries": []}
        rings = []
        for c in g["countries"]:
            for ring in c["rings"]:
                xs = [p[0] for p in ring]
                ys = [p[1] for p in ring]
                rings.append((c["iso"], (min(xs), min(ys), max(xs), max(ys)), ring))
        _COUNTRY_RINGS[key] = rings
    for code, (x0, y0, x1, y1), ring in _COUNTRY_RINGS[key]:
...
def haversine_km(lat1: float, lon1: float, lat2: float, lon2: float) -> float:
    """Great-circle kilometres on a sphere of radius 6371 km. Every distance on this site
    is this number: the flight is longer, the road is longer again, and the pages say so."""
    r = math.radians
